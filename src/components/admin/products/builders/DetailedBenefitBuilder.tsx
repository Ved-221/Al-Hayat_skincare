...
interface DetailedBenefitBuilderProps {
  defaultValue?: DetailedBenefit[] | string | null;
}

const PRESET_ICONS = [
  { icon: "spa", label: "Spa / Herbal" },
  { icon: "leaf", label: "Pure Leaf" },
  { icon: "water_drop", label: "Deep Hydration" },
  { icon: "sparkles", label: "Radiance & Glow" },
  { icon: "verified", label: "Clinically Tested" },
  { icon: "favorite", label: "Self Care" },
  { icon: "clean_hands", label: "Gentle & Pure" },
  { icon: "eco", label: "Organic Herb" },
  { icon: "energy_savings_leaf", label: "Botanical Energy" },
  { icon: "science", label: "Formulated" },
];

export default function DetailedBenefitBuilder({ defaultValue }: DetailedBenefitBuilderProps) {
  const [items, setItems] = useState<DetailedBenefit[]>(() => {
  ...
  const handleAddItem = () => {
    setItems((prev) => [...prev, { icon: "spa", title: "", sub: "" }]);
    setActiveTab("edit");
  };
  ...
  const handleChange = (index: number, field: keyof DetailedBenefit, value: string) => {
    setItems((prev) => {
      const copy = [...prev];
  ...
  return (
    <div className="space-y-4 rounded-xl border border-gray-200 bg-gray-50/50 p-4 sm:p-5">
      ...
      <input type="hidden" name="detailed_benefits" value={JSON.stringify(items)} />

      {/* Header & Tabs */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-gray-200 pb-3">
        <div>
          <h3 className="text-sm font-semibold text-gray-900">
            Key Product Benefits ({items.length})
          </h3>
          <p className="text-xs text-gray-500">
            Highlight primary therapeutic outcomes and skin/hair benefits with icons.
          </p>
        </div>
            ...
            >
              <div className="flex items-start justify-between gap-4 border-b border-gray-100 pb-3 mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-semibold text-gray-700">Benefit #{index + 1}</span>
                </div>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-12">
                <div className="sm:col-span-3">
                  ...
                  <div className="relative">
                    ...
                    <div className="mt-1.5 flex items-center gap-1.5 text-[11px] text-gray-500">
                      ...
                      <span>Icon preview: <strong className="font-mono">{item.icon}</strong></span>
                    </div>
                  </div>
                </div>
                ...
                <div className="sm:col-span-4">
                  ...
                  <input
                    ...
                    placeholder="e.g. Deep Root Nourishment"
                    required
                ...
                <div className="sm:col-span-5">
                  <label className="block text-xs font-medium text-gray-700 mb-1">
                    Benefit Subtitle / Description *
                  </label>
                  <input
                    type="text"
                    value={item.sub}
                    onChange={(e) => handleChange(index, "sub", e.target.value)}
                    placeholder="e.g. Penetrates the scalp to strengthen follicles right from the root up."
                    required
                    className="w-full rounded-lg border border-gray-300 px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-gray-900"
                  />
                </div>
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-xs">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {items.map((item, index) => (
              <div
                key={index}
                className="flex items-start gap-3.5 rounded-xl border border-gray-100 bg-gray-50/70 p-4"
              >
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#434b01]/10 text-[#434b01]">
                ...
                <div>
                  <h5 className="text-xs font-bold text-gray-900">
                    {item.title || `[Benefit #${index + 1} Title]`}
                  </h5>
                  <p className="mt-0.5 text-[11px] leading-relaxed text-gray-600">
                    {item.sub || `[Enter description above to preview...]`}
                  </p>
                </div>
              </div>
            ))}
